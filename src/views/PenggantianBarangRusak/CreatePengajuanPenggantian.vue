...
async function confirmSave() {
  ...
  try {
    ...
    await store.createPengajuan(payload)

    toastStore.success('Success', 'Pengajuan penggantian barang rusak berhasil dibuat')

    router.push('/pengadaan/rusak')

  } catch (error:any) {

    toastStore.error('Error', error || 'Gagal membuat pengajuan')
...
<template>
  <div class="page">
    <div class="mb-8 flex items-center gap-3 margin-bottom: 32px;">
      ...
      <h1 class="title m-0">Buat Pengajuan Penggantian Barang Rusak</h1>
    </div>
  ...
  <ConfirmationModal
    ...
    message="Apakah Anda yakin ingin mengajukan penggantian barang rusak ini?"
