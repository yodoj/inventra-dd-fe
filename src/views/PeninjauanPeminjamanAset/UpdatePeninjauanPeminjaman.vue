...
const handleUpdate = async () => {
  ...
  try {
    // Memanggil updateTinjauan sesuai backlog/store
    await store.updateTinjauan(idPeminjaman.value, {
...
onMounted(async () => {
  ...
  if (idPeminjaman.value) {
    try {
      const data = await store.fetchByPeminjamanId(idPeminjaman.value);
      if (data) {
        form.statusPeminjaman = data.status_peminjaman === 'DIAJUKAN' ? '' : data.status_peminjaman;
        form.alasan = data.alasan || '';
      }
    } catch (err) {
      toastStore.error('Error', 'Gagal memuat data peninjauan lama');
    }
  }
});
...
<template>
  <div class="page">
    <h1 class="title">Ubah Peninjauan Peminjaman Aset</h1>
    ...
    <div class="main-card card-shadow">
      ...
      <div class="review-section">
        <h3 class="section-subtitle">Form Peninjauan (Update)</h3>
  ...
  <ConfirmationModal
    ...
    title="Konfirmasi Update Peninjauan"
    message="Apakah Anda yakin ingin memperbarui data peninjauan ini?"
...
<style scoped>
.page { max-width: 1200px; margin: 40px auto; padding: 0 24px; }
.title { font-size: 28px; font-weight: 800; margin-bottom: 24px; }
.main-card { background: white; padding: 40px; border-radius: 24px; border: 1px solid #eee; }
.section-subtitle { font-size: 20px; font-weight: 700; margin-bottom: 24px; color: #111827; }

.info-grid-2-col { display: grid; grid-template-columns: 1fr 1fr; gap: 40px; }
.row-data { display: grid; grid-template-columns: 160px 10px 1fr; column-gap: 8px; margin-bottom: 12px; align-items: start; }
.label { font-weight: 700; font-size: 14px; color: #111827; }
.value { font-size: 14px; color: #4B5563; word-break: break-word; }

.review-section { margin-top: 40px; padding-top: 32px; border-top: 1px solid #E5E7EB; }
.review-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 40px; align-items: start; }
.field .label { display: block; font-size: 18px; font-weight: 700; margin-bottom: 12px; color: #111827; }

.textarea {
  width: 100%; min-height: 240px; border: 1px solid #D1D5DB; border-radius: 12px;
...
.actions { display: flex; justify-content: center; gap: 24px; margin-top: 48px; }
.btn-submit { background-color: #00588F; color: white; padding: 14px 60px; border-radius: 40px; font-weight: 700; cursor: pointer; border: none; }
.btn-cancel { background-color: #F3F4F6; color: #4B5563; padding: 14px 60px; border-radius: 40px; font-weight: 700; cursor: pointer; border: none; }

.dd { position: relative; }
.dd-btn {
  width: 100%; height: 56px; padding: 0 16px; border: 1px solid #D1D5DB; border-radius: 14px;
  background: white; display: flex; align-items: center; justify-content: space-between; cursor: pointer;
}
.placeholder-color { color: #9CA3AF !important; }
.dd-icon { width: 18px; height: 18px; color: #6B7280; }
.dd-menu {
  position: absolute; left: 0; right: 0; margin-top: 8px; background: white;
  border: 1px solid #E5E7EB; border-radius: 12px; box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.1); z-index: 50; overflow: hidden;
}
.dd-item { width: 100%; text-align: left; padding: 14px 16px; border: none; background: white; cursor: pointer; }
.dd-item:hover { background: #F9FAFB; }

.lock-banner {
  grid-column: span 2; background-color: #fef2f2; border: 1px solid #fee2e2;
  color: #b91c1c; padding: 14px 18px; border-radius: 12px; font-weight: 600;
}

.card-shadow { box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1); }
.required-star { color: #DC3545; }

@media (max-width: 860px) { .info-grid-2-col, .review-grid { grid-template-columns: 1fr; } }
</style>